...
def load_features_and_corr_files(problem_dir):
    ...
    first_corr_arr = np.load(
        f"{problem_dir}/three-body-cubo_first_order_coefs.npy", allow_pickle=True
    )
    second_corr_arr = np.load(
        f"{problem_dir}/three-body-cubo_second_order_coefs.npy", allow_pickle=True
    )
    third_corr_arr = np.load(
        f"{problem_dir}/three-body-cubo_third_order_coefs.npy", allow_pickle=True
    )
...
def find_bruteforce_solution(pcbo, num_threads=1, chunk_size=10):
    ...
    """
    from biomarker_data import _solve_bruteforce
