...
from graph_utils import get_edge_diameter


def generate_dataset_paths(
    ds_param_name: str, ds_param: Sequence, methods: str | list[str]
) -> ndarray:
    """
    ...
    if ds_param_name == "nodes":
        result = np.array(
            [
                f"graphs/main/nodes_{node}/depth_3/output/{method}/out.csv"
                for node in ds_param
        ...
        result = np.array(
            [
                f"graphs/main/nodes_12/depth_{depth}/output/{method}/out.csv"
                for depth in ds_param
                for method in methods
            ]
        )
    else:
...
def fit_data(
    ...
    **kwargs,
    ...
    fit_coeffs, covariance = optimize.curve_fit(
        fit_func,
        xs[fit_ind_range[0] : fit_ind_range[1]],
        ys[fit_ind_range[0] : fit_ind_range[1]],
        **kwargs,
    )
    fitted = fit_func(xs, *fit_coeffs)
    rmse = calc_rmse(
        fitted[fit_ind_range[0] : fit_ind_range[1]],
        ys[fit_ind_range[0] : fit_ind_range[1]],
    )
...
def transfer_expectation_columns(
    df: DataFrame,
    transfer_from: str,
    transfer_to: str,
    angle_suffixes: Sequence[str],
    p_from: int,
    p_to: int,
    transfer_better: bool = True,
) -> DataFrame:
    """
    ...
    for suffix in angle_suffixes:
        ...
        if p_to - p_from == 0:
            df.loc[transfer_rows, transfer_angles_to] = df.loc[
                transfer_rows, transfer_angles_from
            ]
        else:
            original_angles = df.loc[transfer_rows, transfer_angles_from].apply(
                lambda x: numpy_str_to_array(x)
            )
            transformed_angles = [
                str(
                    np.concatenate(
                        (angles, [0] * (len(angles) // p_from) * (p_to - p_from))
                    )
                )
                for angles in original_angles
            ]
...
def transfer_expectation_dataframe(df: DataFrame) -> DataFrame:
    ...
    for p in range(2, max_p + 1):
        df = transfer_expectation_columns(
            df, f"p_{p - 1}", f"p_{p}", ["_angles"], p - 1, p, transfer_better
        )
    return df
...
def merge_dfs(
    ...
    for p_ind, p in enumerate(ps):
        ...
        if (
            next_restarts > calculated_restarts
            and sum(next_df[f"r_{calculated_restarts}"] > convergence_threshold)
            == next_df.shape[0]
        ):
        ...
        next_df = next_df[[exp_col_name, angle_col_name]].set_axis(
            [new_exp_col_name, new_angle_col_name], axis=1
        )
        if merged_df.empty:
            merged_df = next_df
        else:
...
class DataExtractor:
    ...
    def get_cost_worst_case(self) -> Sequence:
        ...
        costs_worst_case = [
            costs_all.iloc[min_inds[col_ind], col_ind]
            for col_ind in range(costs_all.shape[1])
        ]
