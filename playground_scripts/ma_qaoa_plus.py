...
random_energies = [
    evaluate_energy(pcirc, cost_hamiltonian, random_angles) for _ in range(100)
]
...
rounded_angles = np.random.choice(
    np.arange(-np.pi, np.pi + np.pi / 8, np.pi / 8), len(ks_best)
)
rounded_energies = [
    evaluate_energy(pcirc, cost_hamiltonian, rounded_angles) for _ in range(100)
]
...
cafqa_params = [
    param * (multiplier) for param, multiplier in zip(ks_best, angle_multipliers)
]  # This has to be in the order we come across the gates.

# Evaluate Maxcut
max_iters = 1500
random_max_cut_val, random_obj_values, random_fin_energy = evaluate_maxcut(
    G, pcirc, random_angles, cost_hamiltonian, max_iters
)
RA_max_cut_val, RA_obj_values, RA_fin_energy = evaluate_maxcut(
    G, pcirc, rounded_angles, cost_hamiltonian, max_iters
)
cafqa_max_cut_val, cafqa_obj_values, cafqa_fin_energy = evaluate_maxcut(
    G, pcirc, cafqa_params, cost_hamiltonian, max_iters
)
