...
def main():
    ...
    for n in ns:
        ...
        for run in range(num_runs):
            ...
            red_solver = QAOASolver(red_cost, red_circ.decompose().decompose(), sim_device="CPU")
            ...
            eval_solver = QAOASolver(cost, circ.decompose().decompose(), sim_device="CPU")
            eval_solver.vanilla = True
            try:
                red_energy = eval_solver.evaluate_energy(eval_solver.circuit, eval_solver.cost_hamiltonian, best_red)
                if red_energy > 0:
            ...
            try:
                ...
                qaoa_obj.run_CAFQA(n_gens=5, out_file=None) #TODO: increase gens when needed
        ...
        with open(summary_log, "a") as f:
            f.write(f"rerun (without skipping) {n} qubits: geomean_red={geomean_red}, std_red={std_red}, geomean_cafqa={geomean_cafqa}, std_cafqa={std_cafqa}\n")
        print(f"rerun (without skipping)   n={n} done: geomean_red={geomean_red}, std_red={std_red}, geomean_cafqa={geomean_cafqa}, std_cafqa={std_cafqa}")
