...
srun --cpu-bind=cores --output=../logs/Comprehensive_Proof/Multi-Start/Gradient_Norm/Maxcut/$N_QUBITS/noisy/task_%t_%j.log python gradient_norm_select_maxcut.py $N_QUBITS $N_REPS $NUM_GENERATIONS $SEED $NOISE 
...
# srun --cpu-bind=cores --output=../logs/Comprehensive_Proof/Multi-Start/Gradient_Norm/Teague/$N_QUBITS/task_%t_%j.log python /global/u1/d/dhanvib/development/QAOA/teague_code/code-for-gokul/gradient_norm_select_teague.py $N_QUBITS $N_REPS $NUM_GENERATIONS $SEED $NOISE 
