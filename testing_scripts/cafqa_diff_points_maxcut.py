...
def run_qaoa_task_pool(args):
    ...
    try:
        ...
        result, obj_values = maxcut_qaoa.run_qaoa(initial_params=cafqa_params, max_iters=max_iters, opt="COBYQA")
...
def execute_qaoa_tasks(ma_qaoa_object, vanilla_qaoa_object, selected_cafqa_parameters, selected_fitness_values):
    ...
    random_args = [("Random MA-QAOA",ma_qaoa_object,random_angles,selected_fitness_values)]

    # Prepare task for Vanilla QAOA
    random_vanilla_angles = np.random.random(vanilla_qaoa_object.circuit.num_parameters)
    vanilla_args = [("Vanilla_task", vanilla_qaoa_object, random_vanilla_angles, selected_fitness_values)]
...
def main():
    ...
    # vanilla_res,vanilla_obj_vals=vanilla_maxcut.run_qaoa(vanilla_random_vanilla_angles, max_iters=max_iters)

    start = timer()
    results = execute_qaoa_tasks(maxcut_qaoa, vanilla_maxcut, selected_cafqa_parameters, selected_fitness_values)
    end = timer()
    ...
    np.save(os.path.join(output_dir, f"COBYQA_point_analysis_{seed}.npy"), results)
