...
def main():
    ...
    start = timer()
    results = execute_qaoa_tasks(knapsack_qaoa, vanilla_knapsack, selected_cafqa_parameters, selected_spaced_fitness_vals)
    end = timer()
    ...
    np.save(os.path.join(output_dir, f"SPSA_best_spaced_5_point_analysis_{seed}.npy"), results)
