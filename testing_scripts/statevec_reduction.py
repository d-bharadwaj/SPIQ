...
def compare_cafqa_vs_random_distribution(num_qbs, seed):
    sv_data = np.load(
        f"../np_data/Comprehensive_Proof/Maxcut/k_reg/2_reps/{num_qbs}_qbs/cafqa_result_{seed}.npy",
        allow_pickle=True,
    ).item()
    ...
    cafqa_dist_len = len(
        graph_utils.get_final_distribution(qaoa_obj, cafqa_angles, shots=1e6)
    )
    random_dist_len = len(
        graph_utils.get_final_distribution(qaoa_obj, random_angles, shots=1e6)
    )
    print(
...
def geomean_cafqa_vs_random_distribution(num_qbs, num_seeds=10):
