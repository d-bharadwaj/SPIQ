...
from _values import pubo_value, pubo_value_safe, puso_value, qubo_value, quso_value
...
def process_mini_chunk(args):
    x_list, D_list, value, spin = args
    if spin:
        D = qv.PUSO({tuple(term[0]): term[1] for term in D_list})
    else:
        D = qv.PCBO({tuple(term[0]): term[1] for term in D_list})
    mini_best = None, {}
    for x in x_list:
        v = value(x, D)
        if mini_best[0] is None or v < mini_best[0]:
            mini_best = v, x
    return mini_best


def divide_chunk(input_list, m):
    n = len(input_list)
...
def _solve_bruteforce(D, all_solutions, valid, spin, value, ncpus=1, chunk_size=1000):
    """_solve_bruteforce.
    ...
    value : function.
        ...
        ``puso_value``.
    ncpus : int (optional, defaults to 1).
    ...
    """
    all_solutions = False  # we're setting this to False by default for ease of implementation and memory efficiency
    if not D:
    ...
    for test_sol in itertools.product((1, -1) if spin else (0, 1), repeat=N):
        ...
        if len(cur_chunk) < chunk_size - 1:
            cur_chunk.append(x)
        else:
            cur_chunk.append(x)
            # Multiprocessing
            n = len(cur_chunk) // ncpus + 1
            with multiprocessing.Pool(ncpus) as pool:
                args = [
                    ...
                    for mini_chunk in divide_chunk(cur_chunk, ncpus)
                    if len(mini_chunk) > 0
                ]
                results = pool.map(process_mini_chunk, args)
            # Get the best soln(s) out of the chunk
            for cur_v, cur_x in results:
                if best[0] is None or cur_v < best[0]:
                    best = cur_v, cur_x
            cur_chunk = []
    else:
...
def solve_puso_bruteforce(
    ...
    """
    return _solve_bruteforce(
        H, all_solutions, valid, True, puso_value, ncpus=ncpus, chunk_size=chunk_size
    )
