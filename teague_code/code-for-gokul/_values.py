...
__all__ = ("pubo_value", "qubo_value", "puso_value", "quso_value")
...
def pubo_value_safe(x, P):
    ...
    """
    # map is faster than all(x[i] for i in k)
    return sum(v for k, v in P.items() if all(map(Checker(x), k)))
...
def qubo_value(x, Q):
    ...
    return sum(
        v
        for k, v in Q.items()
        if (not k or (len(k) == 1 and x[k[0]]) or (len(k) == 2 and x[k[0]] and x[k[1]]))
    )
...
def puso_value(z, H):
    ...
    """
    return sum(v * pow(-1, [z[i] for i in k].count(-1) % 2) for k, v in H.items())
